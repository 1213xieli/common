...
public class RedisUtils {
     ...
     * @return
     */
    public Set<String> sStringGet(String key) {
        try {
     ...
     * @return 成功个数
     */
    public long sStringSet(String key, String... values) {
    	if(values==null||values.length==0){
    		return 0;
    	}
        try {
     ...
     * @return 成功个数
     */
    public long sStringSetAndTime(String key, long time, String... values) {
    	if(values==null||values.length==0){
    		return 0;
    	}
        try {
            Long count = this.sStringSet(key, values);
            if (time > 0)
                expire(key, time);
            return count;
        } catch (Exception e) {
